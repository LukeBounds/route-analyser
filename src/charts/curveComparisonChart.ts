import { maximumValue, numericBounds, prepareCanvas, widestText } from './canvas.js';
...
export function drawCurveComparisonChart(options: CurveComparisonChartOptions) {
    ...
    const X = drawGradeGrid(context, width, height, minGrade, maxGrade, left, right, top, bottom);
    const Y = metric === 'pace'
    ...
    const V = showVam ? drawVamScale(context, width, height, left, right, top, bottom, maxVam, showVamGuides) : null;
