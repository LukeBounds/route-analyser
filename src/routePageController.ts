...
function predictionDifferenceText(actualSeconds: number, differenceSeconds: number) {
    const predictedSeconds = actualSeconds - differenceSeconds;
    if (!(predictedSeconds > 0))
        return signedDuration(differenceSeconds);
    const percentage = differenceSeconds / predictedSeconds * 100;
    const roundedPercentage = Math.abs(percentage) < .05 ? 0 : percentage;
    const sign = roundedPercentage > 0 ? '+' : roundedPercentage < 0 ? '−' : '';
    return `${signedDuration(differenceSeconds)} (${sign}${Math.abs(roundedPercentage).toFixed(1)}%)`;
}
...
function activityViewAccessor(): ActivityViewAccessor | null {
    ...
    return {
        ...
        cumulativeDifferenceAt: distance => activityComparison(activity[0].routeD, distance)?.delta ?? null,
    };
}
...
function terrainRowHtml(row: TerrainRowViewModel, showPrediction: boolean, showActivity: boolean) {
    ...
    const actual = showActivity
        ? row.actual
            ? `<td class="analysis-column-group-start">${durationText(row.actual.seconds)}</td><td>${row.actual.paceSecondsPerKm === null ? '—' : `${formatPace(row.actual.paceSecondsPerKm)}/km`}</td><td>${row.actual.vamMetersPerHour === null ? '—' : vamText(row.actual.vamMetersPerHour, 3600)}</td><td>${durationText(row.actual.cumulativeSeconds)}</td><td class="analysis-column-group-start prediction-difference">${predictionDifferenceText(row.actual.seconds, row.actual.differenceSeconds)}</td><td class="prediction-difference">${predictionDifferenceText(row.actual.cumulativeSeconds, row.actual.cumulativeDifferenceSeconds)}</td>`
...
function terrainSummaryActual(summary: TerrainAggregateViewModel, showActivity: boolean, cumulative: boolean, showVam: boolean) {
    ...
    const difference = summary.actualDifferenceSeconds === null ? '—' : predictionDifferenceText(summary.actualSeconds, summary.actualDifferenceSeconds);
    return `<th class="analysis-column-group-start">${durationText(summary.actualSeconds)}</th><th>${pace}</th><th>${vam}</th><th>${cumulative ? durationText(summary.actualSeconds) : ''}</th><th class="analysis-column-group-start prediction-difference">${difference}</th><th class="prediction-difference">${cumulative ? difference : ''}</th>`;
}
...
function renderWaypointSegments() {
    ...
    const rows = viewModel.rows.map(row => {
        const actual = showActivity
            ? row.actual
                ? `<td class="analysis-column-group-start">${row.actual.paceSecondsPerKm === null ? '—' : `${formatPace(row.actual.paceSecondsPerKm)}/km`}</td><td>${row.actual.vamMetersPerHour === null ? '—' : vamText(row.actual.vamMetersPerHour, 3600)}</td><td>${durationText(row.actual.seconds)}</td><td>${durationText(row.actual.cumulativeSeconds)}</td><td class="analysis-column-group-start prediction-difference">${predictionDifferenceText(row.actual.seconds, row.actual.differenceSeconds)}</td><td class="prediction-difference">${predictionDifferenceText(row.actual.cumulativeSeconds, row.actual.cumulativeDifferenceSeconds)}</td>`
                : '<td class="analysis-column-group-start">—</td><td>—</td><td>—</td><td>—</td><td class="analysis-column-group-start">—</td><td>—</td>'
            : '';
    ...
    const overallActual = showActivity
        ? activityTotal
            ? `<th class="analysis-column-group-start">${activityDistance > 0 ? `${formatPace(activityTotal.actual / (activityDistance / 1000))}/km` : '—'}</th><th>${vamText(activityChange, activityTotal.actual)}</th><th>${durationText(activityTotal.actual)}</th><th>${durationText(activityTotal.actual)}</th><th class="analysis-column-group-start prediction-difference">${predictionDifferenceText(activityTotal.actual, activityTotal.delta)}</th><th class="prediction-difference">${predictionDifferenceText(activityTotal.actual, activityTotal.delta)}</th>`
